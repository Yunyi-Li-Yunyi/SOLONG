...
class adniData(Dataset):
    ...
    def __init__(self,csv_file,var1,var2,t1,t2):
        self.adni = pd.read_csv(csv_file)
        self.SparseData=[]
        valid1 = self.adni[(np.isnan(self.adni[var1]) == 0)
                           & (np.isnan(self.adni[t1]) == 0)
                           & (self.adni[t1]>=0)]
                           # & (self.adni[args.age]!= 0)]
                               # & ((self.adni['DX.bl'] =='CN')|(self.adni['DX.bl'] =='SMC'))]
        valid1['normalized'] = preprocessing.scale(valid1[var1])
        filter1=valid1['APOE4'].isin([1,2])
        valid1=valid1[filter1]
        print(valid1["APOE4"])

        valid2 = self.adni[(np.isnan(self.adni[var2]) == 0)
                           & (np.isnan(self.adni[t2]) == 0)
                           & (self.adni[t2]>=0)]
                           # & (self.adni[args.age]!= 0)]
                              # & ((self.adni['DX.bl'] == 'CN') | (self.adni['DX.bl'] == 'SMC'))]
        valid2['normalized'] = preprocessing.scale(valid2[var2])
        ...
        t1 = torch.tensor(valid1[t1].to_numpy())
        ...
        t2 = torch.tensor(valid2[t2].to_numpy())
...
if __name__ == "__main__":
    ...
    dataset = adniData(cvs_file,var1,var2,t1,t2)
    ...
    predX = np.load(folder + '/predX_1000.npy', allow_pickle=True)
    minT = min(time)
    maxT = max(time)
    ...
    plt.legend()
    plt.savefig(folder + "/AdniPlot_v2_epoch100" )
