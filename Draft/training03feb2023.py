...
class Initfunc(nn.Module):
    def __init__(self,x0):
        super(Initfunc, self).__init__()
        outdim=x0.size(0)
        self.linear = nn.Linear(1,outdim,bias=False)
        with torch.no_grad():
            self.linear.weight.copy_(x0)
        ...
        for i, data in enumerate(data_loader):
            ...
            if self.sim == True and self.ts_equal == True:
                ...
                if param == "theta":
                    ...
                    updatedx0 = torch.reshape(updatedx0,(2, 1))
