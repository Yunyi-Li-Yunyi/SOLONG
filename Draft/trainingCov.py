...
class Trainer:
    ...
    def train(self, train_data_loader: DataLoader, epochs: int, seed):
        ...
        for epoch in range(epochs):
            ...
            if self.sim == True:
                epoch_loss = self.train_epoch(train_data_loader, epoch, seed, betaHat)
            else:
    ...
    def train_epoch(self, data_loader, epoch, seed,betaHat):
        epoch_loss = 0.
        # self.train()
        ...
        for i, data in enumerate(data_loader):
            ...
            if self.sim == True and self.ts_equal == True:
                t, x_obs, x_true, covariate = data[:][1]
                t = t.to(self.device)
                x_obs = x_obs.to(self.device)
                x_true = x_true.to(self.device)
                covariate = covariate.type(torch.float32).to(self.device)
                x_obs_cov = x_obs - betaHat*covariate[:,None,None]
                # sort_t, sort_x_obs, sort_x_true = od(t, x_obs_cov, x_true)
                # X0 = x_obs[:,0,:]
                # sort_t = sort_t.to(self.device)
                # sort_x_obs = sort_x_obs.to(self.device)
                # pred_x = odeint(self.ODEFunc, X0, t[]).to(self.device)
                pred_x=[]
                for i in range(t.size(0)):
                    x0 = x_obs[i,0,:]
                    pred_x.append(odeint(self.ODEFunc, x0, t[i]))
                pred_xt = torch.cat(pred_x,dim=0).view(x_obs_cov.size())
                ymat = x_obs_cov - pred_xt
                crep = covariate.repeat_interleave(t.size(1)*2)[:,None]
                ct = torch.transpose(crep,0,1)
                betaHat = torch.inverse(ct@crep)@ct@torch.reshape(ymat, (-1,1))
                print('betaHat......',betaHat)
                pred_xt += betaHat*covariate[:,None,None]

                loss = torch.mean(torch.square(pred_xt - x_obs))
                loss.backward()
                self.optimizer.step()
