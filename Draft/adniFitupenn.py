...
class adniData(Dataset):
    ...
    def __init__(self,csv_file,var1,var2):
        ...
        self.var2=var2
        self.SparseData=[]
        self.adni["Age"]=round(self.adni["Age"],1)
        var1_valid = self.adni[(np.isnan(self.adni[self.var1]) == 0)
                               & (np.isnan(self.adni.Age) == 0)]
                               # & ((self.adni["DX.bl"].isin(['AD','EMCI','LMCI']))
                               # | (self.adni["DX"].isin(['Dementia','MCI'])))]
...
dataset = adniData('/data/draftData/upenn.csv'
                   ,'ABETA','TAU')

data_loader = DataLoader(dataset)
sim=False
ts_equal=False
folder='/N/slate/liyuny/cnode_ffr_main/results/adni/abeta_tau_lr4_100_4l_all/'
